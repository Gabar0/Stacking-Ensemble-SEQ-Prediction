...
"""Notbook_Stacking_Ensemble_model_no WTW_15.08.26.ipynb
"""

# Comprehensive Stacking_Ensemble Model Evaluation: Includes train-test splits, cross-validation, bootstrap validation, Residual analysis, Error distribution, and Bland-Altman analysis.
...
train_sd = np.std(y_train - y_train_pred, ddof=1)
test_sd = np.std(y_test - y_pred, ddof=1)

overfit_table = pd.DataFrame({
    'Metric': ['MAE (D)', 'MSE (D²)', 'R²', 'SD (D)'],
    'Training Set': [train_mae, train_mse, train_r2, train_sd],
    'Test Set': [test_mae, test_mse, test_r2, test_sd],
})

overfit_table['Difference'] = (overfit_table['Test Set'] - overfit_table['Training Set']).abs()
overfit_table['Ratio'] = overfit_table['Test Set'] / overfit_table['Training Set']

print("\n=== Training vs Test Performance (Overfitting Check) ===")
print(overfit_table.to_string(index=False, formatters={
    'Training Set': '{:.4f}'.format,
    'Test Set': '{:.4f}'.format,
...
plt.tight_layout()
#save .png
plt.savefig('GENERATING PLOTS', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
# save .png
plt.savefig('RESIDUAL PATTERN ANALYSIS', dpi=150, bbox_inches='tight')
plt.show()
...
def quick_overfit_plot(estimator, X_train, y_train, X_test, y_test):
    ...
    plt.tight_layout()
    plt.savefig('VISUAL OVERFITTING CHECK', dpi=150, bbox_inches='tight')
    plt.show()
...
def quick_learning_curve(estimator, X, y, train_sizes=[0.3, 0.6, 1.0], cv=3):
    ...
    plt.savefig('LEARNING CURVE', dpi=150, bbox_inches='tight')
    plt.show()
...
def bootstrap_overfit_check(estimator, X, y, n_bootstraps=50, test_size=0.2):
    ...
    plt.grid(True, alpha=0.3)
    plt.savefig('BOOTSTRAP OVERFITTING CHECK', dpi=150, bbox_inches='tight')
    plt.show()
...
def plot_al_groups_performance(performance_df):
    ...
    plt.tight_layout()
    plt.savefig('AL GROUP ANALYSIS', dpi=150, bbox_inches='tight')
    plt.show()
...
plt.savefig('COMPREHENSIVE MODEL EVALUATION', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('SHAP SUMMARY PLOT', dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('FEATURE IMPORTANCE BAR PLOT', dpi=150, bbox_inches='tight')
plt.show()
...
for name, model in stacking_reg.named_estimators_.items():
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(f'Summary plot for individual model - {name}', dpi=150, bbox_inches='tight')
        plt.show()
...
for feature in top_features:
    try:
        ...
        plt.tight_layout()
        plt.savefig(f'Dependence plot for {feature}', dpi=150, bbox_inches='tight')
        plt.show()
    except Exception as e:
...
try:
    ...
    plt.tight_layout()
    plt.savefig(f'Waterfall plot for sample {sample_idx}', dpi=150, bbox_inches='tight')
    plt.show()
